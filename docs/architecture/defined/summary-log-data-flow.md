...
- [Waste balance (pEPR LLD)](pepr-lld.md#waste-balance)
...
| **Waste balance ledger**              | Prior transactions with `source.summaryLogRow.wasteRecordId` matching the row                                   | Per-row reconciliation — the signed sum of prior ledger transactions for the same waste record gives the amount already credited; a new transaction is appended only when the target amount (from current waste-record state) differs |
...
flowchart TD
    ...
    T --> WR["Waste Records\nupdated with\nnew versions"]:::auto
    T --> WB["Waste Balance ledger\nappended: one transaction\nper balance-affecting row\nfor the signed delta\n(or none if zero)"]:::auto
...
| --------------------------------- | ------------------------- | ------------------------------------------------------- | -------------------------------- | -------------------- | ---------------------- |
| **Summary Log submitted**         | Updated (new versions)    | Auto-corrected (per-row ledger append)                  | Auto-corrected                   | **Stale** — recreate | —                      |
...
2. **On next submission** — Each balance-affecting row is re-classified against the current accreditation state; if the target amount differs from what the ledger has already credited for that waste record, one transaction is appended for the signed difference. Rows already at the correct amount emit nothing. Changes to accreditation dates or suspension status are **not reflected in the waste balance until the operator uploads a new summary log**.
