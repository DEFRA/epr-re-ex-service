...
- **Other uploads.** Any other upload for the registration that was validated against the previous submission is refused when the operator tries to submit it, with a message to upload again.
...
- **Uploads in progress.** Any upload for the registration created before the report was submitted fails at submission, whichever periods it touches. The submit request is accepted, and the upload then shows as a failed submission with no reason recorded. The operator uploads again.
...
- **Rejection.** Computed reports drop the PRN at once, but the balance is restored only when the cancellation completes, so the two disagree until then.
- **Cancellation.** A cancellation without a rejection restores the balance at once.
- **Reports after a rejection or cancellation.** The in-progress or ready-to-submit report for the month the PRN was issued is marked stale. The flag is set after the PRN is saved, and is not retried if it fails. A submitted report for that month is not flagged, and keeps counting the PRN unless the operator asks to resubmit it.
...
- **PRNs.** Issuing is refused at once, while drafting continues during a suspension. Drafting is refused once the accreditation is cancelled, but an existing draft can still be raised and ringfence balance it can never issue. Existing PRNs keep their copied details.
- **Reports.** Suspension leaves reports alone: figures do not read accreditation status, and cadence stays monthly. Cancellation moves the operator to quarterly periods and to the registered-only report category. That category finds received loads by a date only the registered-only template carries, while validation still requires the accredited template. So a computed report counts none of the received loads, and no upload can bring them back. Monthly reports already stored keep their figures but drop out of the operator's list.

Cancelling a registration cancels a linked approved or suspended accreditation, with the same effects.

### Accreditation granted

- **Uploads.** Once the accreditation has a number, an upload validated from then on must use the accredited template, or validation rejects it. An upload validated before the grant is not checked again when it is submitted.
...
- **Uploads.** Validation checks an upload against the new material and processing type, and rejects a mismatch outright. An upload validated before the change is not checked again when it is submitted.
...
- **Stored reports.** They keep the site details as they were when created. Nothing flags them, so an unsubmitted report stays out of date until the operator deletes and recreates it, and a submitted one until the operator asks to resubmit it.
- **Site names on rows.** A row holds the site name the operator typed, so a change to the site's name does not reach it.
...
| **Report submitted**                     | —                                                                  | —                                                                  | Earlier ones **fail** at submission, no reason recorded                             | —                                                                                 | Period closed; later changes flag it                                                  | —                                                   | Stored-report listings on next read                 |
...
| **PRN rejected or cancelled**            | Restored when cancellation completes                               | —                                                                  | —                                                                                   | PRN dropped at once                                                               | Issue month's in-progress or ready report **marked stale**; submitted **not flagged** | —                                                   | Next read                                           |
| **Accreditation dates changed**          | Out of date until next submission                                  | Out of date until next submission                                  | —                                                                                   | Figures unaffected; monthly periods due move                                      | Unaffected                                                                            | Keep copied details                                 | Reclassified at once; can **disagree** with balance |
| **Accreditation suspended or cancelled** | Out of date until next submission                                  | Out of date until next submission                                  | —                                                                                   | Suspension: unaffected. Cancellation: quarterly; received loads no longer counted | Unaffected; cancellation drops monthly ones from the list                             | Issue refused; cancellation also refuses new drafts | Reclassified at once; can **disagree** with balance |
| **Accreditation granted**                | New stream from first submission                                   | —                                                                  | Accredited template required from next validation; continuity baseline starts empty | Monthly, accredited category; registered-only rows drop out                       | Quarterly reports drop out of the list                                                | —                                                   | Read every stream                                   |
| **Registration details changed**         | Unaffected                                                         | Unaffected                                                         | Checked against the new values from next validation                                 | Current material and site; category from current processing type                  | Current material and site shown                                                       | Keep copied details                                 | Next read                                           |
| **Organisation details changed**         | Unaffected                                                         | Unaffected                                                         | —                                                                                   | Unaffected                                                                        | Unaffected                                                                            | Keep copied name                                    | Next read                                           |
| **Overseas site data changed**           | Out of date until next submission if approval or site list changed | Out of date until next submission if approval or site list changed | —                                                                                   | Next read                                                                         | Out of date, not flagged                                                              | —                                                   | Reclassified at once; can **disagree** with balance |
