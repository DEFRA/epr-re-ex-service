<!-- GENERATED FILE - DO NOT EDIT BY HAND. Generated by DEFRA/epr-backend scripts/generate-summary-log-rules.mjs. To update, re-run the tool. -->
...
When a rule's trigger holds for a row anywhere in the Summary Log, all of its required fields must be filled or the Monthly Report cannot be created. See the [report-creation gate](report-creation-mandatory-fields.md) for the rationale.

| Rule | Sheet | Trigger | Required fields |
| ---- | ----- | ------- | --------------- |
| `supplier_details` | Exported | Positive TONNAGE_RECEIVED_FOR_EXPORT | `SUPPLIER_NAME`, `SUPPLIER_ADDRESS`, `SUPPLIER_POSTCODE`, `SUPPLIER_EMAIL`, `SUPPLIER_PHONE_NUMBER`, `ACTIVITIES_CARRIED_OUT_BY_SUPPLIER` |
| `overseas_site` | Exported | Positive TONNAGE_OF_UK_PACKAGING_WASTE_EXPORTED | `OSR_ID` |
| `export_date` | Exported | Positive TONNAGE_OF_UK_PACKAGING_WASTE_EXPORTED | `DATE_OF_EXPORT` |
| `interim_site` | Exported | DID_WASTE_PASS_THROUGH_AN_INTERIM_SITE = Yes | `INTERIM_SITE_ID` |
| `final_destination` | Sent on | Positive TONNAGE_OF_UK_PACKAGING_WASTE_SENT_ON | `FINAL_DESTINATION_NAME`, `FINAL_DESTINATION_FACILITY_TYPE`, `FINAL_DESTINATION_ADDRESS`, `FINAL_DESTINATION_POSTCODE` |

#### Exported sheet (`exported`)

This section feeds the Waste Balance. A row here contributes its tonnage unless it is held back for one of these reasons (see [reason meanings](summary-log-row-validation-classification.md)):

- `MISSING_REQUIRED_FIELD` (excluded)
- `OUTSIDE_ACCREDITATION_PERIOD` (ignored)
- `WASTE_STOPPED` (excluded)
- `WASTE_REFUSED` (excluded)
- `ORS_NOT_FOUND` (excluded)
- `ORS_NOT_APPROVED` (excluded)
- `PRN_ISSUED` (excluded)
...
| Rule | Sheet | Trigger | Required fields |
| ---- | ----- | ------- | --------------- |
| `supplier_details` | Received (section 1) | Positive TONNAGE_RECEIVED_FOR_EXPORT | `SUPPLIER_NAME`, `SUPPLIER_ADDRESS`, `SUPPLIER_POSTCODE`, `SUPPLIER_EMAIL`, `SUPPLIER_PHONE_NUMBER`, `ACTIVITIES_CARRIED_OUT_BY_SUPPLIER` |
| `overseas_site` | Exported (sections 2 and 3) | Positive TONNAGE_OF_UK_PACKAGING_WASTE_EXPORTED | `OSR_ID` |
| `export_date` | Exported (sections 2 and 3) | Positive TONNAGE_OF_UK_PACKAGING_WASTE_EXPORTED | `DATE_OF_EXPORT` |
| `final_destination` | Sent on (section 4) | Positive TONNAGE_OF_UK_PACKAGING_WASTE_SENT_ON | `FINAL_DESTINATION_NAME`, `FINAL_DESTINATION_FACILITY_TYPE`, `FINAL_DESTINATION_ADDRESS`, `FINAL_DESTINATION_POSTCODE` |
...
| Rule | Sheet | Trigger | Required fields |
| ---- | ----- | ------- | --------------- |
| `supplier_details` | Received | Positive TONNAGE_RECEIVED_FOR_RECYCLING | `SUPPLIER_NAME`, `SUPPLIER_ADDRESS`, `SUPPLIER_POSTCODE`, `SUPPLIER_EMAIL`, `SUPPLIER_PHONE_NUMBER`, `ACTIVITIES_CARRIED_OUT_BY_SUPPLIER` |
| `final_destination` | Sent on | Positive TONNAGE_OF_UK_PACKAGING_WASTE_SENT_ON | `FINAL_DESTINATION_NAME`, `FINAL_DESTINATION_FACILITY_TYPE`, `FINAL_DESTINATION_ADDRESS`, `FINAL_DESTINATION_POSTCODE` |
...
- `MISSING_REQUIRED_FIELD` (excluded)
- `OUTSIDE_ACCREDITATION_PERIOD` (ignored)
- `PRODUCT_WEIGHT_NOT_ADDED` (excluded)
