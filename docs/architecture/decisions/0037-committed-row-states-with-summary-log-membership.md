...
**Per-submission snapshot artefacts: the parsed rows of the whole workbook, gzipped in S3, keyed by `summaryLogId`.** An earlier draft of this ADR. Immutable by construction, a single idempotent put on the commit path, zero MongoDB growth. Rejected in favour of membership documents: it adds a second storage system and an artefact format to the committed path, every committed read pays a fetch-gunzip-parse, bulk reads become a per-registration artefact fan-out, and row-level access needs a separate projection built by diffing consecutive artefacts — machinery the membership collection replaces with one multikey index. What the artefact had over membership is _structural_ immutability; the event gate carries the commit semantics either way, so that immutability is defence in depth, and we trade it for the simplicity of one queryable store whose invariants (states never mutated, arrays only grow, comparison anchored to the committed head) are enforced by write-path discipline and contract tests.
...
- **One storage system.** No artefact format, no S3 read path, no projection to keep fresh — the committed row state is directly queryable where the rest of the data lives.
...
- **Correctness is disciplinary, not structural.** The invariants — state documents never mutate, membership arrays only grow, comparison anchors to the committed head, every write is an idempotent upsert — live in write-path code. A bug can violate them in place, where an immutable artefact could not be wrong at rest; they need contract-test enforcement.
- **The commit path keeps a many-document bulk write** (one operation per row) ahead of the event append — the same cost the submission path pays today, but the snapshot alternative would have shrunk it to a single put.
