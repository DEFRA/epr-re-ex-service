...
**Actor vs cause.** `createdBy` is the **actor** — the user who triggered the transaction — and is uniform across every transaction kind. For summary-log-row transactions it is stamped from the SubmitUser at submit time, so every row in the same submission carries the same `createdBy` (per-batch actor, not per-row author). For PRN operations it is the user who performed the operation. For manual adjustments it is the user who made the adjustment. `source` is the **cause** — the upstream event and the entity it affected, discriminated by `source.kind` and varying in shape per kind. The actor is never carried inside `source`. An actor query ("show me every transaction user X has caused") resolves uniformly against `createdBy` across all kinds; a cause query resolves uniformly against `source.*` without having to dig the actor out of three different nested shapes.

**Naming convention.** Fields nested inside a reference object are entity-scoped state — the path provides the scope, so the names drop redundant entity prefixes (`type` not `rowType`, `versionId` not `wasteRecordVersionId`). Transaction-root closing totals (`closingAmount`, `closingAvailableAmount`) are accreditation-wide and stand without a containing object, so they keep the `closing*` prefix to mark them as transaction-temporal snapshots. Closing totals at the root, running totals inside their entity sub-object, named for what they are without temporal or entity qualifiers.
...
PRN operations key on `prnId` and carry their delta directly; manual adjustments carry their delta directly without a per-entity reconciliation key. The read-before-emit invariant applies only to summary-log-row writes.
...
**Compute `alreadyCredited` via signed-sum aggregation over prior matching transactions, without persisting a per-waste-record running total.** Rejected because the closing-totals discipline (`opening + delta = closing` carried on every transaction) was applied only to the global running totals, not to per-waste-record tracking. The aggregation primitive carries signed-sum semantics, sign handling, pending-debit filtering, and `accreditationId` scoping — discipline that lives outside the ledger and leaks into both the contract surface and the consumer's reasoning. Persisting `wasteRecord.creditedAmount` on each summary-log-row transaction makes the per-row read primitive symmetrical with the balance read primitive (`find-latest-by-key`), and the pending-debit filter disappears because PRN pending debits do not carry a `source.summaryLogRow` path. Cost is one `Decimal128` field per summary-log-row transaction.

**Keep `source.summaryLogRow` flat with `rowId, rowType, wasteRecordId, wasteRecordVersionId` as siblings, with `wasteRecordId` synthesised as `${rowType}:${rowId}`.** Rejected because the synthesised string carries no information not already in `(rowType, rowId)` and introduces a load-bearing `:` delimiter convention into the database; because three `wasteRecord`-prefixed fields at the same nesting level (`wasteRecordId`, `wasteRecordVersionId`, plus the new running credited total) are more prefix-noise than structure; and because nesting waste-record state inside a `wasteRecord` sub-object lets each field name lose its redundant entity prefix (`type` not `rowType`, `versionId` not `wasteRecordVersionId`) — the path provides the scope. The PAE-1380 rowId-collision fix is preserved as a compound `(type, rowId)` key.

**Drop `createdBy` and resolve the actor by joining `source.summaryLogRow.summaryLogId` → system-logs (or equivalent join for other source kinds).** Reduces per-transaction duplication — every summary-log-row transaction in a batch carries the same SubmitUser, derivable from the summary log header. Rejected because cross-source actor queries ("show me what user X did across summary-log uploads, PRN operations, and manual adjustments") are the more useful read pattern, and resolving them via per-kind joins replaces a single `createdBy` lookup with three different join shapes the consumer has to know about. The duplication cost is bounded — one user id per transaction — and the read ergonomics are uniform across kinds.
