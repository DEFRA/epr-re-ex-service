...
**Actor vs cause.** `createdBy` is the **actor** — the user who triggered the transaction — and is uniform across every transaction kind. For summary-log-row transactions it is stamped from the SubmitUser at submit time, so every row in the same submission carries the same `createdBy` (per-batch actor, not per-row author). For PRN operations it is the user who performed the operation. `source` is the **cause** — the upstream event and the entity it affected, discriminated by `source.kind` and varying in shape per kind. The actor is never carried inside `source`. An actor query ("show me every transaction user X has caused") resolves uniformly against `createdBy` across both kinds; a cause query resolves uniformly against `source.*` without having to dig the actor out of two different nested shapes.
...
PRN operations key on `prnId` and carry their delta directly. The read-before-emit invariant applies only to summary-log-row writes.
...
**Drop `createdBy` and resolve the actor by joining `source.summaryLogRow.summaryLogId` → system-logs (or equivalent join for other source kinds).** Reduces per-transaction duplication — every summary-log-row transaction in a batch carries the same SubmitUser, derivable from the summary log header. Rejected because cross-source actor queries ("show me what user X did across summary-log uploads and PRN operations") are the more useful read pattern, and resolving them via per-kind joins replaces a single `createdBy` lookup with per-kind join shapes the consumer has to know about. The duplication cost is bounded — one user id per transaction — and the read ergonomics are uniform across kinds.
