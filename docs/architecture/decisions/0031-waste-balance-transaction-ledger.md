...
**Amended 2026-05-01:** split actor (`createdBy`) and cause (`source`) explicitly. `createdBy` is the uniform actor field on every transaction (stamped from the SubmitUser for summary-log rows at submit time — per-batch, not per-row); `source.manualAdjustment` carries `reason` only, with the actor moving to `createdBy` alongside every other transaction kind. No production data exists under this ADR yet, so the change is amend-in-place rather than superseding.
