...
`closedPeriodAdjustments` is not yet enabled in production, so unlike `stale`'s migrate-on-read fix, this needs no legacy-shape normalisation — the flat shape is changed directly, in the same PR that adds the new field. That PR also updates the one in-repo reader of the old flat path: `src/reports/monitoring/pre-cpa-resubmission.js`'s docstring references `resubmissionRequired.summaryLogId`, which becomes `resubmissionRequired.closedPeriodRestated.summaryLogId`.

**Every write path holds one invariant: `resubmissionRequired` is never persisted as an empty `{}`, only ever with at least one trigger key set.** Existing readers that test the field's bare presence — `assertResubmissionAllowed` (`report-service.js`), gating draft creation on `Boolean(previous?.resubmissionRequired)`, and `build-calendar-periods.js`'s "requires resubmission" branch — depend on this invariant now holding by construction rather than, as with the flat shape, by the field simply not existing until populated. Both gates are updated alongside the shape change to check key presence explicitly (`Object.keys(resubmissionRequired ?? {}).length > 0`) rather than lean on bare truthiness, so they stay correct even if a future write path breaks the invariant.
