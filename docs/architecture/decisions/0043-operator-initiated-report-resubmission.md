...
`closedPeriodAdjustments` is not yet enabled in production, so unlike `stale`'s migrate-on-read fix, this needs no legacy-shape normalisation — the flat shape is changed directly, in the same PR that adds the new field. The implementing PR also needs to update the one in-repo reader of the old flat path, `src/reports/monitoring/pre-cpa-resubmission.js`, whose docstring references `resubmissionRequired.summaryLogId` (becomes `resubmissionRequired.closedPeriodRestated.summaryLogId`).

**Invariant this shape change introduces, and who relies on it:** `resubmissionRequired` is only ever written with at least one trigger key set — no write path in this design ever persists an empty `{}`. `assertResubmissionAllowed` (`report-service.js`) currently gates draft creation on `Boolean(previous?.resubmissionRequired)`, which is correct only because the flat shape today is never present-but-empty. Once the field is a container, "field present" and "at least one trigger set" are no longer the same fact by construction — they only coincide because every write path upholds the invariant above. The implementing PR must hold that gate to the same invariant explicitly, e.g. `Object.keys(previous?.resubmissionRequired ?? {}).length > 0`, rather than leave it resting on bare truthiness. The same class of check applies anywhere else `resubmissionRequired`'s presence (not a specific sub-key) is tested — `build-calendar-periods.js`'s "requires resubmission" branch is the other existing example, worth reviewing in the implementing PR though its resolution can be decided there.
