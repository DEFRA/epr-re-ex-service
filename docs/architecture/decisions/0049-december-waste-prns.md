...
Every existing arithmetic rule from ADR-0036 stays byte-for-byte the same — the rules keep moving `amount` and `availableAmount` as the total. A thin December layer is added on top, keyed off a `useDecemberBalance` flag carried on the event:

- PRN events carry a distinct `useDecemberBalance` flag, separate from the PRN's `isDecemberWaste` disclosure marker. This is the one place the ADR introduces a dimension tag, and it does so deliberately: a reprocessor-output PRN self-declares `isDecemberWaste: true` for the statutory disclosure yet must not touch a December pool, so reusing the PRN's flag on the event would force the event to carry `isDecemberWaste: false` and state the opposite of the PRN it belongs to. `useDecemberBalance` is resolved once, when the PRN's first balance event is written: `useDecemberBalance = isDecemberWaste && (the accreditation accrues December capacity)`. Every balance event the PRN subsequently produces (`prn-issued`, and any reversal) carries its own copy of that resolved value; no event re-derives it or reads it from a sibling event. For exporter and reprocessor-input accreditations the two coincide; a reprocessor-output PRN's event always carries `useDecemberBalance: false`, whatever the PRN self-declares, because output accreditations accrue no December capacity (see Scope). Events with `useDecemberBalance: true` additionally move `decemberAmount` / `decemberAvailableAmount` by the same delta they already apply to the total; other events touch only the total fields. Reversals read the event's `useDecemberBalance`, so a cancelled December-pool PRN credits the December fields, restoring December capacity. Snapshotting `useDecemberBalance` onto the event (resolved once from the PRN flag and the accreditation, rather than re-read per event) keeps each event self-contained and replay-safe, consistent with ADR-0036's actor-vs-cause principle.
...
- **The `useDecemberBalance` flag on PRN events.** Every PRN balance event carries `useDecemberBalance`, resolved once from the PRN's `isDecemberWaste` and whether the accreditation accrues December; the close layer routes on it. The set of kinds is unchanged; only the payload widens.
